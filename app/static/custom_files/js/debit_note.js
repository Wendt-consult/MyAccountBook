...
$("#debit_date").datepicker({dateFormat: 'dd-mm-yy'}).datepicker("setDate", new Date(),dateFormat = "dd-mm-yy");
...
/*********************************************************************** */
// DEFUALT DEBIT NOTE AND CHECK DEBIT NOTE NUMBER IS UNIQUE
/*********************************************************************** */
$("#auto_debit_number").click(function(){
    if($(this).is(':checked')){
        var ins = 0
        var slug = 'a'
        $.ajax({
            type:"GET",
            url: "/debitnote/unique_number/"+ins+"/"+slug+"/",
            dataType: "json",
            success: function(data){
                $('#debit_number').val(data.debit_note)
            },
            error: function (rs, e) {
...
$("#debit_number").focusout(function(){
    $("#auto_debit_number").prop('checked',false);
    var ins = 1
    var debit_number = $("#debit_number").val()
    $.ajax({
        type:"GET",
        url: "/debitnote/unique_number/"+ins+"/"+debit_number+"/",
        dataType: "json",
        success: function(data){
            if(data.unique != 0){
                alert('This debit note number is already exits. Please enter the different debit note number.')
                $('#debit_number').focus();
                $('#debit_number').val('')
            }
...
var global_debit_number = 1
var count = 0

function add_single_row(a){
    if(count == 0){
        global_debit_number +=a
        count +=1
    }else{
        global_debit_number += 1
    }
    run_others();
}
function run_others(){
    ...
    next_id = global_debit_number;       
    html = product_row_creator(next_id);
    $('#debit_table').append(html);
    ret = get_func(next_id,acc_group_name_htm, products_htm);

}
function product_row_creator(global_debit_number){
    ...
    var html = '<tr id="debit_row'+global_debit_number+'">'
    html +='<td style="border:1px solid black;padding-bottom:0%"><select class="form-control select debit_line_item" id="ItemName'+global_debit_number+'" name="ItemName[]" onchange="product('+global_debit_number+'),validation()" style="padding-left:0px"><option value="">None</option></select>'
    html +='<textarea id="desc'+global_debit_number+'" name="desc[]" rows="2" maxlength="200" size="200" placeholder="Product Description" style="width: 174.6px;margin-top:1px;"></textarea></td>'
    html +='<td style="border:1px solid black;"><select class="form-control prodduct_debit_account" id="product_account'+global_debit_number+'" name="product_account[]" onchange="validation()"><option value="">None</option></select></td>'
    html +='<td style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right:0%"><label for="Price1">₹</label></div>'
    html +='<div class="col"><input type="text" class="form-control" maxlength="10" onkeypress="return restrictAlphabets(event), float_value(event,\'Price'+global_debit_number+'\')" onkeyup="debit_calculate('+global_debit_number+'),validation()" id="Price'+global_debit_number+'" name="Price[]" style="margin-top:1%"></div></div></td>'
    html +='<td style="border:1px solid black;"><input type="text" class="form-control" maxlength="10" id="Quantity'+global_debit_number+'" onkeypress="return restrictAlphabets(event), float_value(event,\'Quantity'+global_debit_number+'\')" onkeyup="debit_calculate('+global_debit_number+'),validation()" name="Quantity[]"></td>'
    html +='<td style="border:1px solid black;"><input class="form-control" id="Unit'+global_debit_number+'" name="Unit[]" style="padding-left:0px;" readonly></td>'
    html +='<td style="border:1px solid black;"><div class="row"><div class="col-7" style="padding-right:3px;"><input type="text" class="form-control all_discount" maxlength="10" onkeypress="return restrictAlphabets(event), float_value(event,\'Discount'+global_debit_number+'\')" onkeyup="debit_calculate('+global_debit_number+')" id="Discount'+global_debit_number+'" name="Discount[]"></div>'
    html += '<div class="col-5" style="padding-left:1px;"><select class="form-control" id="Dis'+global_debit_number+'" name="Dis[]" onchange="dicount_type('+global_debit_number+')" style="background-color: white;color: black;padding-left:0%;"><option value="%">%</option><option value="₹">₹</option></select></div></div></td>'
    html +='<td style="border:1px solid black;"><div class="row"><div class="col-8" style="padding-right:3px"><input list="tax" class="form-control tax" maxlength="5" size="5" onkeyup="row_gst_cal('+global_debit_number+')" onkeypress="return restrictAlphabets(event), float_value(event,\'tax'+global_debit_number+'\')" name="tax[]" id="tax'+global_debit_number+'" style="margin-top:-1px" readonly><datalist id="tax"><option value="0"><option value="5"><option value="12"><option value="18"><option value="28"></datalist></div>'
    ...
    if(org_state != '' || vendor_g != '' || org_state == vendor_g || $('#org_gst_reg_type').val() == ''){
        html += '<td class="row_cs_gst" style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_cgst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_cgst" id="row_cgst'+global_debit_number+'" name="row_cgst[]" style="margin-top: 1%;" readonly></div></div></td>'
        html +='<td class="row_cs_gst" style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_sgst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_sgst" id="row_sgst'+global_debit_number+'" name="row_sgst[]" style="margin-top: 1%;" readonly></div></div></td>'
        html +='<td class="row_i_gst" style="border:1px solid black;display:none;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_igst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_igst" id="row_igst'+global_debit_number+'" name="row_igst[]" style="margin-top: 1%;" readonly></div></div></td>'
    }else if(org_state != vendor_g){
        html += '<td class="row_cs_gst" style="border:1px solid black;display:none;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_cgst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_cgst" id="row_cgst'+global_debit_number+'" name="row_cgst[]" style="margin-top: 1%;" readonly></div></div></td>'
        html +='<td class="row_cs_gst" style="border:1px solid black;display:none;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_sgst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_sgst" id="row_sgst'+global_debit_number+'" name="row_sgst[]" style="margin-top: 1%;" readonly></div></div></td>'
        html +='<td class="row_i_gst" style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right: 0%;"><label for="row_igst'+global_debit_number+'">₹</label></div><div class="col"><input type="text" class="form-control row_igst" id="row_igst'+global_debit_number+'" name="row_igst[]" style="margin-top: 1%;" readonly></div></div></td>'    
    }
    html +='<td style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right:0%"><label for="Amount'+global_debit_number+'">₹</label></div>'
    html +='<div class="col"><input type="text" class="form-control amount" id="Amount'+global_debit_number+'" name="Amount[]" style="margin-top:1%;" readonly></div></div></td>'
    html +='<td style="border:1px solid black;"><div class="row"><div class="col-1" style="padding-right:0%"><label for="Amount_inc'+global_debit_number+'">₹</label></div>'
    html +='<div class="col"><input type="text" class="form-control amount_inc" id="Amount_inc'+global_debit_number+'" name="Amount_inc[]" style="margin-top:1%;" readonly></div></div></td>'
    html +='<td style="border-top: none;"><span class="tbclose material-icons" id="'+global_debit_number+'" name="'+global_debit_number+'" onclick=" return creditnote_removeRow('+global_debit_number+')" style="cursor: default;">delete_forever</span></td></tr>'
    return html;
}

function get_func(next_id, acc_group_name_htm, products_htm){

    $.get("/debitnote/add_debit_note/"+1+"/", function(data){
        if(data){
            ...
            $(document).on('click','#select2-ItemName'+global_debit_number+'-container',function(){

                for(var i = 1;i <= global_debit_number; i++){
                    var a = $('#row_ItemName'+global_debit_number+'').length
                    if(a == 0){
                        $('.select2-search').append('<button class="btn btn-link purchase_product" data-toggle="modal" id="row_ItemName'+global_debit_number+'" onclick="get_purchase_product_id('+global_debit_number+'),c()" data-target="#ProductModal" style="margin-left: -11%;">+ Add New</button>');
                    }  
                } 
            });
        
            $(document).on('click','#select2-product_account'+global_debit_number+'-container',function(){
        
                for(var i = 1;i <= global_debit_number; i++){
                    var a = $('#row_account'+global_debit_number+'').length
                    if(a == 0){
                        $('.select2-search').append('<button class="btn btn-link purchase_account" data-toggle="modal" id="row_account'+global_debit_number+'" onclick="get_purchase_account_id('+global_debit_number+'),c()" data-target="#addGroupModal" style="margin-left: -24%;">+ Add New</button>');
                    }  
                } 
            });
            ...
            $(function () {
                $(".prodduct_debit_account").select2();
...
function creditnote_removeRow(a) {
    var first_row = $('#debit_table tbody tr:first').attr('id')
    if(first_row == 'debit_row'+a+''){
        var last_row = $('#debit_table tbody tr:last').attr('id')
        if(last_row != 'debit_row'+a+''){
            $('#debit_row'+a+'').remove();
        }else{
...
//         var a = $('#row_ItemName'+global_debit_number+'').length
//         if(a == 0){
//         $('.select2-search').append('<button class="btn btn-link debit_product" data-toggle="modal" id="row_ItemName'+global_debit_number+'" onclick="get_debit_product_id('+global_debit_number+'),c()" data-target="#ProductModal" style="margin-left: -11%;">+ Add New</button>');
//         }  
//     } 
// });
...
function get_debit_product_id(ids){
    prefill_debit_product = ids

}
var prefill_debit_product 
...
//         var a = $('#row_account'+global_debit_number+'').length
//         if(a == 0){
//         $('.select2-search').append('<button class="btn btn-link debit_account" data-toggle="modal" id="row_account'+global_debit_number+'" onclick="get_debit_account_id('+global_debit_number+'),acc_product_account()" data-target="#addGroupModal" style="margin-left: -35%;">+ Add New</button>');
//         }  
//     } 
// });
...
function get_purchase_account_id(ids){
    prefill_debit_account = ids

}
var prefill_debit_account 
...
$(document).ready(function() {
    ...
    $(function () {
        $("#debit_vendor").select2();
      });
    });
// $(document).on('click','#select2-debit_vendor-container',function(){
...
/********************************************************************/
// DEBIT NOTE PRODUCT MODEL SAVE USING AJAX
/********************************************************************/

function debit_product_form(save_type){
        ...
        success: function(data) {
            if(data != '0'){
                $.get("/creditnote/product_fetch/",function(data){
                    $(".debit_line_item").each(function(){
                        $('<option/>').val(data.ids).html(data.name).appendTo($(this));
                    });
                    $('#ItemName'+prefill_debit_product+'').val(data.ids).change(); 
    ...
    $.post("/contacts/add/",$("#add_purchase_contact_form").serialize(), function(data){
        if(data != '0'){
            ...
            $.get("/creditnote/contact_fetch/",function(data){
                if(category == 'vendor'){
                    $("#debit_vendor").each(function(){
                        $('<option/>').val(data.ids).html(data.name).appendTo($(this));
                    });
                    $('#debit_vendor').val(data.ids).change(); 
                }
...
/********************************************************************/
// DEBIT NOTE ACCOUNT_LEDGER(GROUP) MODEL SAVE USING AJAX
/********************************************************************/
function debit_accoun_ledger_form(save_type){
    ...
    if(data != '0'){
        ...
        $.get("/purchase_order/account_group_fetch/",function(data){
            ...
            $('#product_account'+prefill_debit_account+'').val(data.ids).change(); 
...
function total_discount(){
    ...
    if(discount.toString() != 'NaN'){
        $('#debit_Discountotal').val(parseFloat(discount).toFixed(2))
    }
    if(discount == 0){
        $('#debit_Discountotal').val('')
    }
}
...
function check_gst_status(cat){
    if(cat == 'user_side'){
        ...
        if($('#org_gst_reg_type').val() == '0' || $('#org_gst_reg_type').val()  =='3' || $('#org_gst_reg_type').val()  == '5' || $('#org_gst_reg_type').val()  == '' ||  $('#gst_type').val() == '0' ||  $('#gst_type').val() == '3' ||  $('#gst_type').val() == '5' ||  $('#gst_type').val() == ''){
            $('#debit_table').find('.tax').attr('readonly', true)
            $('#debit_table').find('.tax,.row_cgst,.row_sgst,.row_igst').val('')
            sub_total()
            ...
            $('#debit_table').find('.tax').attr('readonly', false)
            sub_total()
        }
...
/********************************************************************/
// debit note SGST CGST AND IGST CALCULATION 
/********************************************************************/

function debit_tax_cacultion(){
    var org_state = $('#single_gst_code option:selected').val()
    var vendor_g = ''
    if(vendor_gstin != ''){
        vendor_g = vendor_gstin.substring(0,2)
...
function change_state(){
    ...
    if(org_state != '' & vendor_g != ''){
        if(org_state == vendor_g || $('#org_gst_reg_type').val() == ''){
            $('#debit_table').find('.row_i_gst').hide()
            $('#debit_table').find('.row_igst').val('')
            $('#debit_table').find('.row_cs_gst').show()
            $('#debit_table').find('.price_header').css('width','7%')
            $('#debit_table').find('.unit_header').css('width','11%')
            $('#debit_table').find('.quantity_header').css('width','5%')
            $('#debit_table').find('.tax_header').css('width','7%')
        }else if(org_state != vendor_g){
            $('#debit_table').find('.row_cs_gst').hide()
            $('#debit_table').find('.row_cgst, .row_sgst').val('')
            $('#debit_table').find('.row_i_gst').show()
            $('#debit_table').find('.price_header').css('width','8%')
            $('#debit_table').find('.unit_header').css('width','12%')
            $('#debit_table').find('.quantity_header').css('width','6%')
            $('#debit_table').find('.tax_header').css('width','8%')
        }
    }
    $(".debit_line_item").each(function(){
            ...
            success: function(data){
                ...
                for(var i = 0;i < count.length;i++){
                    ...
                    var html = '<tr class="address_row" id="purchase_delivery_address_row_'+i+'"><td style="border:1px solid black;" align="center"><label class="form-check-label">'
                        ...
                        $('#debit_delivery_address').append(html)
                        
                }

            },
        });
        $('#debit_order_AddressModal').modal('show')
    }
}

//  CONTACT ADDRESS
function debit_cont_address(){
...
function prefill(){
    ...
    $('#debit_address').text(address)
    $('#debit_attention').val(global_parson.substring(16,global_parson.length))
    // $('#order_state').val(global_delivery_state)
    // check_gst_status('delivary_side')
    
    $('#debit_order_AddressModal').modal('hide')
}
            ...
            if(data != '0'){
                ...
                $.get("/purchase_order/last_address/",function(data){
                    ...
                    if(data.contact_person != null){
                        $('#debit_attention').val(data.contact_person)
...
function check_mail(){
    if($('#debit_address').text() == ''){
        ...
        alert('can not send mail because vendor email id did not exist')
        return false
    }else if(global_save_message != ''){
        alert(global_save_message)
        return false
...
$('#debit_date').keypress(function(event) {
        event.preventDefault();
});
...
function validation(){
    var count = $('#debit_table tbody tr').length
    var row_ids = $('#debit_table tbody tr:first').attr('id')
...
$("#debit_date").on('change', function(event){
    dateValidation($(this));
});
...
$('.quantity_dic').keyup(function(){
    ...
    if(parseFloat($(this).val()) > parseFloat(dataDic[quant])){
        alert('You can not increase quantity')
        $('#'+quant).val(dataDic[quant])
        quant = quant.match(/\d+/);
        quant = quant[0]
        debit_calculate(quant)
