...
class ExpenseForm(forms.ModelForm):
	def __init__(self, *args, **kwargs):
		...
		self.fields['vendor'].queryset = Contacts.objects.filter(user=user._wrapped)
		self.fields['payment_method'].queryset = PaymentMethod.objects.filter(user=user._wrapped)

	vendor = forms.ModelChoiceField(queryset=Contacts.objects.none(), widget=forms.Select(attrs={'class':'form-control input-sm col-md-7', 'id':'select_vendor'}))
