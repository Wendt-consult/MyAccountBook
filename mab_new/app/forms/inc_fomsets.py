...
AddressFormset = inlineformset_factory(contacts_model.Contacts, users_model.User_Address_Details, extra = 2,
    ...
    widgets = {
        ...
        'city' : TextInput(attrs={'class':'form-control input-sm','required':True,'style':'width:65%;padding-left:9px;'}),
        'state' : Select(attrs={'class':'form-control input-sm','required':True,'style':'width:65%;'}, choices = country_list.STATE_LIST_CHOICES),
        'country' : Select(attrs={'class':'form-control input-sm','required':True,'style':'width:65%;'}, choices = country_list.COUNTRIES_LIST_CHOICES),
